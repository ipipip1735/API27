...
//        TransitionManager.go(fourScene, transtionSet.setDuration(5000L));
        ...
        TransitionSet set = new TransitionSet()
                ...
                .setOrdering(ORDERING_TOGETHER) //并行播放
        ...
        imageView.setX(imageView.getX() + 500f);//布局属性要有改变MoveTransition才起作用
        imageView.setY(imageView.getY() + 350f);

        ViewGroup.LayoutParams layoutParams = imageView.getLayoutParams();
        layoutParams.height += 450f;
        layoutParams.width += 150f;
        imageView.setLayoutParams(layoutParams);//布局面积要有改变ChangeBounds才起作用
...
//        transition.addTarget(root.getChildAt(2)); //加入白名单
    ...
    private void visibility() {
        ...
        //使用Fade变换
//        Transition fade = new Fade() {//进出都动画
////        Transition fade = new Fade(IN|OUT) {//等价于无参构造函数
////        Transition fade = new Fade(0) {//禁用动画
////        Transition fade = new Fade(IN) {//仅进场动画
////        Transition fade = new Fade(OUT) {//仅出场动画
//            @Override
...
////        Transition transition = new Slide(TOP){
//            @Override
//            public Animator onAppear(ViewGroup sceneRoot, View view, TransitionValues startValues, TransitionValues endValues) {
//                System.out.println("~~onAppear~~");
//
...
//        transitionManager.transitionTo(twoScene);
        ...
        transitionManager = new TransitionManager();
        transitionManager.setTransition(fiveScene, explode.setDuration(3000L));//任意源场景
        transitionManager.transitionTo(fiveScene);
    ...
    private void customTransition() {
        ...
        oneScene.setExitAction(new Runnable() {
            @Override
            public void run() { //绑定出场钩子函数
                System.out.println("oneScence exit!!");
            }
        });
        oneScene.setEnterAction(new Runnable() {
            @Override
            public void run() { //绑定进场钩子函数
                System.out.println("oneScence enter!!");}
        });
        twoScene.setExitAction(new Runnable() {
            @Override
            public void run() { //绑定出场钩子函数
                System.out.println("twoScene exit!!");}
...
//                findViewById(R.id.cl2).setVisibility(View.INVISIBLE);
        ...
        twoScene.setEnterAction(new Runnable() {
            @Override
            public void run() { //绑定进场钩子函数
                System.out.println("twoScene enter!!");
